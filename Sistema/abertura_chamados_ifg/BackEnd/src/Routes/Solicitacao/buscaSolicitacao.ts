 ...
 * /buscaSolicitacao:
 *   get:
 *     summary: Retorna uma lista paginada de chamados
 *     tags:
 *       - Solicitações
 *     parameters:
 *       - in: query
...
const buscaSolicitacao = Router();


buscaSolicitacao.get("/buscaSolicitacao", async (req: Request, res: Response) => {
  const pagina = parseInt(req.query.page as string) || 1; // Página atual (default: 1)
  const limitePorPagina = 2; // Quantidade de registros por página
...
export default buscaSolicitacao;
